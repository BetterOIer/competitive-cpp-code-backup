...
int ols()
{
	...
	for(int i=2;i<=50000;i++)//这个意思是在100001里面找到质数并且标记 ，质数最小是2，所以i=2 
	{
		if (a[i]==0)	b[++k]=i;	//如果没有被标记为1，就是质数。我接下来会讲解为什么是质数。 
		for(int j=1;j<=k;j++)//j小于当前所有的质数的个数
		{
			if(i*b[j]>50000)break;// 如果超出给出的范围，那么就退出循环 
...
void init(){
    for(int i = 1;i<=5133;i++)all[i]=0;
    for(int i = 1;i<=5133;i++)maxn[i]=0;
    for(int i = 1;i<=5133;i++)minn[i]=2147483647;
}
void solve(){
    int n=read();
    for(int i = 1,x;i<=n;i++){

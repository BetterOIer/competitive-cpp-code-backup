...
int main(){
    for(int i = 1;i<=200000;i++){
        ...
        for(int j = 1;j<=i;j++){
            int cnt=2^j;
            if(cnt<j){/* cout<<ans[i]<<" "<<i<<" "<<j<<"->"<<cnt<<endl; */tot++;}
        }
        for(long long o = 62;o>=0;o--){
            if(2>=table[o]){clo=table[o];break;}
        }
        an+=(clo*(i/(2*clo)));
        an+=(i-(i/(2*clo)*(2*clo))+1>clo? i-(i/(2*clo)*(2*clo))+1-clo:0);
        if(tot!=an){cout<<tot<<" "<<an<<" "<<(tot==an)<<endl;break;}
    }
    return 0;
}
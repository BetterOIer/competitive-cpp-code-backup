...
int head[maxn],c[maxn<<2],id[maxn],son[maxn];
int fa[maxn],dep[maxn],top[maxn],siz[maxn];
int cnt=0;
int n,m,u,v;
...
void dfs1(int u,int f){
	...
	for(int i=head[u];i;i=a[i].next){
		...
		dfs1(a[i].v,u);
		siz[u]+=siz[a[i].v];
		if(siz[a[i].v]>t)t=siz[a[i].v],son[u]=a[i].v;
	}
}
int cnt1=0;
void dfs2(int u,int f){
	...
	dfs2(son[u],f);
	for(int i=head[u];i;i=a[i].next){
		if(a[i].v==fa[u]||a[i].v==son[u])continue;
		dfs2(a[i].v,a[i].v);
	}
	return ;
}
